...
int RSACipher::EncryptAsBase64(const unsigned char* in, size_t inlen, std::string& out)
{
    std::vector<uint8_t> ret;
    int retcode = Encrypt(in, inlen, ret);
    if ( retcode > 0)
    {
        out = Utils::Base64Encode(ret.data(), ret.size());
    }
    return retcode;
}

int RSACipher::DecryptFromBase64(const std::string& in, std::vector<uint8_t>& out)
{
    std::vector<uint8_t> data;
    if (Utils::Base64Decode(in, data))
    {
        return Decrypt(data.data(), data.size(), out);
    }
    mLastErrorMsg.assign("Base64 decode failed");
    return -1;
}
